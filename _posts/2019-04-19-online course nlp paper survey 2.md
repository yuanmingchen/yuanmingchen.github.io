...
当学生从一个子论坛转到另一个子论坛时，他们在与不同兴趣、目标和关注点相关的讨论中与不同的学生子集交互时，可能会采取不同的立场。从论坛的结构来看，可以构建一个基于线程内回复-评论结构的社交网络图。这种网络结构为学生在MOOC中的社会参与提供了一种视角，它可能反映了学生的价值观和目标。在这些讨论中，学生们所讲的课文提供了一种补充的观点。在我们的建模方法中,我们把这两种来源的洞察力在一起成一个联合估计集成框架的目标建模语言选择的方式由学生在讨论反映特定立场他们承担根据它们相互作用,因此为他们哪些:最突出的。
